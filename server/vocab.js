...
export async function getWord(wordId) {
    ...
    const nearRows = await queryDatabase(
        ...
        UNION
        (SELECT w.id, w.text FROM near_words JOIN word w ON word1_id = w.id WHERE word2_id = $1)`, [wordId]);
    ...
    const confusionRows = await queryDatabase(
        ...
        UNION
        (SELECT w.id, w.text, nuance FROM confusion JOIN word w ON word1_id = w.id WHERE word2_id = $1)`, [wordId]);
